...
describe("PizzaToken contract", function() {
    // global vars
    let Token;
    let oceanToken;
    let owner;
    ...
    beforeEach(async function () {
        Token = await ethers.getContractFactory("PizzaToken"); // creating instance of PizzaToken contract
        [owner, addr1, addr2] = await hre.ethers.getSigners(); // making wallets
    
        oceanToken = await Token.deploy(tokenCap, tokenBlockReward);
      });

      describe("Deployment", function () { // ensure state of vars is correct
      ...
      describe("Transactions", function () { // testing transactions work properly
        it("Should transfer tokens between accounts", async function () {
          ...
          await oceanToken.connect(addr1).transfer(addr2.address, 50); // .connect(signer) to send a transaction from another account
